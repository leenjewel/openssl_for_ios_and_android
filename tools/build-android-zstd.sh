...
function configure_make() {
    ...
    pushd .
    cd "${LIB_NAME}"
    # cp -f ../Makefile.zstd.lib lib/Makefile

    PREFIX_DIR="${pwd_path}/../output/android/zstd-${ABI}"
    if [ -d "${PREFIX_DIR}" ]; then
        rm -fr "${PREFIX_DIR}"
    fi
    mkdir -p "${PREFIX_DIR}"

    OUTPUT_ROOT=${TOOLS_ROOT}/../output/android/zstd-${ABI}
    mkdir -p ${OUTPUT_ROOT}/log

    set_android_toolchain "zstd" "${ARCH}" "${ANDROID_API}"
    set_android_cpu_feature "zstd" "${ARCH}" "${ANDROID_API}"
